...
static int init_display(struct fbtft_par *par)
{
	...
	/* Set frame rate */
	write_reg(par, SEPS114A_OSC_ADJUST,0x00);              // frame rate : 95Hz
...
static void set_addr_win(struct fbtft_par *par, int xs, int ys, int xe, int ye)
{
	...
	write_reg(par, SEPS114A_MEMORY_WRITE_READ,0x02);
	// Column address
	write_reg(par, SEPS114A_MEM_X1, xs);
	write_reg(par, SEPS114A_MEM_X2, xe);

	// Row adress
	write_reg(par, SEPS114A_MEM_Y1, ys);
	write_reg(par, SEPS114A_MEM_Y2, ye);
}

static void seps114a_display_reset(struct fbtft_par *par) {
	if (par->gpio.reset != -1) {
		gpio_set_value(par->gpio.reset, 0);  /* Activate chip */
		mdelay(5);
		gpio_set_value(par->gpio.reset, 1);  /* Activate chip */
		mdelay(5);
	}
}
inline u16 swap_rgb (u16 rgb)
{
	return =	(((rgb << 8) & 0xff00) |
				((rgb >> 8) & 0xff));
}
static int write_vmem(struct fbtft_par *par, size_t offset, size_t len)
{
	...
	for (x = 0; x < par->info->var.xres; x++) {
		for (y = 0; y < par->info->var.yres; y++) {
			vmem16[y*par->info->var.xres+x] = swap_rgb(vmem16[y*par->info->var.xres+x]);
		}
	}
	...
	par->fbtftops.write(par, vmem16, par->info->var.xres*par->info->var.yres*2);
